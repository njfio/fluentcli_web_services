        ...
        <div class="flex items-center space-x-4">
          ...
          <div class="relative">
            <input type="text" placeholder="Search..."
              class="bg-primary-700 text-white placeholder-primary-300 rounded-full py-2 px-4 focus:outline-none focus:ring-2 focus:ring-primary-300 dark:bg-gray-700 dark:placeholder-gray-400"
              @focus="isSearchFocused = true" @blur="isSearchFocused = false">
            <svg class="absolute right-3 top-2.5 h-5 w-5 text-primary-300 dark:text-gray-400"
              :class="{ 'hidden': isSearchFocused }" fill="none" viewBox="0 0 24 24" stroke="currentColor">
              <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
          ...
          <div class="relative" v-if="user">
            <button @click="toggleUserMenu"
              class="flex items-center text-white focus:outline-none hover:text-primary-200 transition-colors duration-200">
              <img :src="user.avatar || 'https://www.gravatar.com/avatar/?d=mp'" alt="User avatar"
            ...
            <transition name="fade">
              <div v-if="showUserMenu"
                ...
                <a href="#"
                  class="block px-4 py-2 text-sm text-gray-700 dark:text-gray-200 hover:bg-primary-500 hover:text-white transition-colors duration-200">Profile</a>
                <a href="#"
                  class="block px-4 py-2 text-sm text-gray-700 dark:text-gray-200 hover:bg-primary-500 hover:text-white transition-colors duration-200">Settings</a>
                <a @click="logout"
                  class="block px-4 py-2 text-sm text-gray-700 dark:text-gray-200 hover:bg-primary-500 hover:text-white transition-colors duration-200 cursor-pointer">Logout</a>
              </div>
            </transition>
          </div>
        </div>
